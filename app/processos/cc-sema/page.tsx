...
export default function CcSemaPage() {
  ...
  // Obter todos os processos CC-SEMA do contexto
  const ccSemaProcesses = getProcessesByType("CC-SEMA")
  
  // Obter processo selecionado para detalhes
  const selectedProcessData = selectedProcess ? ccSemaProcesses.find(p => p.id === selectedProcess) : null
  
  // Obter processo para upload
  const uploadProcess = uploadProcessId ? ccSemaProcesses.find(p => p.id === uploadProcessId) : null
  
  // Obter processo para documentos
  const documentsProcess = documentsProcessId ? ccSemaProcesses.find(p => p.id === documentsProcessId) : null
  ...
  const handleUpload = async () => {
    ...
    const currentProcess = ccSemaProcesses.find(p => p.id === uploadProcessId)
    if (currentProcess) {
  ...
  const generateDocumentsList = (processId: string, documentsCount: number) => {
    const docTypes = ['Certidão de Conformidade', 'Auditoria Ambiental', 'Relatório de Conformidade', 'Termo de Compromisso', 'Registro de Atividades', 'Plano de Gestão', 'Declaração Ambiental']
    const documents = []
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-foreground">CC-SEMA</h1>
                <p className="text-muted-foreground">Certidão de Conformidade Ambiental</p>
              </div>
            </div>
          </div>
          <NewProcessDialog 
            defaultType="CC-SEMA"
            buttonText="Novo CC-SEMA"
          />
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card className="border-yellow-200 bg-yellow-50 dark:bg-yellow-950/20">
            <CardHeader className="pb-3">
              <CardTitle className="text-yellow-700 dark:text-yellow-300 text-lg">Total de Processos</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-yellow-800 dark:text-yellow-200">{ccSemaProcesses.length}</div>
              <p className="text-sm text-yellow-600 dark:text-yellow-400">Processos CC-SEMA ativos</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-green-200 bg-green-50 dark:bg-green-950/20">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-green-800 dark:text-green-200">
                {ccSemaProcesses.filter(p => p.status === "Aprovado").length}
              </div>
              <p className="text-sm text-green-600 dark:text-green-400">Certidões concedidas</p>
            </CardContent>
          </Card>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Certidão de Conformidade Ambiental - Acompanhe todos os processos de certificação
                </CardDescription>
              </div>
              <Badge variant="outline" className="text-yellow-700 border-yellow-200">
                {ccSemaProcesses.length} processos
              </Badge>
            </div>
          </CardHeader>
          <CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <h4 className="font-medium text-foreground">O que é o CC-SEMA?</h4>
                <p className="text-sm text-muted-foreground">
                  A Certidão de Conformidade Ambiental (CC-SEMA) é um documento que atesta a conformidade 
                  das atividades desenvolvidas com as normas ambientais vigentes.
                </p>
              </div>
              <div className="space-y-2">
                <h4 className="font-medium text-foreground">Documentos Necessários</h4>
                <ul className="text-sm text-muted-foreground space-y-1">
                  <li>• Certidão de conformidade</li>
              ...
              {uploadProcess && (
                <div className="p-3 bg-yellow-50 rounded-md">
                  <div className="flex items-center gap-2 mb-2">
                    <FileText className="h-4 w-4 text-yellow-600" />
                    <span className="font-medium text-sm">CC-SEMA</span>
                  </div>
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-green-50 rounded-md">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <FileText className="h-5 w-5 text-yellow-600" />
                      <span className="font-medium">CC-SEMA - {documentsProcess.id}</span>
                    </div>
                ...
                {/* Documentos Necessários (Específicos para CC-SEMA) */}
                <div className="space-y-2">
                  <Label className="text-sm font-medium text-yellow-700">Documentos Necessários para CC-SEMA</Label>
