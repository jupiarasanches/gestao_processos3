...
import { 
  MapPin, Eye, Upload, Calendar, User, FileText, Clock, CheckCircle, X, Download, PauseCircle, AlertTriangle
} from "lucide-react"
...
export default function GeorreferenciamentoPage() {
  ...
  // Obter todos os processos Georreferenciamento do contexto
  const geoProcesses = getProcessesByType("Georreferenciamento")
  
  // Obter processo selecionado para detalhes
  const selectedProcessData = selectedProcess ? geoProcesses.find(p => p.id === selectedProcess) : null
  const uploadProcess = uploadProcessId ? geoProcesses.find(p => p.id === uploadProcessId) : null
  const documentsProcess = documentsProcessId ? geoProcesses.find(p => p.id === documentsProcessId) : null
  ...
  const handleUpload = async () => {
    ...
    const currentProcess = geoProcesses.find(p => p.id === uploadProcessId)
    if (currentProcess) {
  ...
  const generateDocumentsList = (processId: string, documentsCount: number) => {
    const docTypes = ['Memorial Descritivo', 'Planta Topográfica', 'Coordenadas Geográficas', 'Levantamento GPS', 'Relatório Técnico', 'Marco Geodésico', 'Certidão INCRA']
    const documents = []
      ...
      <div className="flex-1 space-y-6 p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-3xl font-bold text-foreground">Georreferenciamento</h1>
                <p className="text-muted-foreground">Demarcação e Coordenadas Geográficas</p>
              </div>
            </div>
          </div>
          <NewProcessDialog 
            defaultType="Georreferenciamento"
            buttonText="Novo Georreferenciamento"
          />
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card className="border-blue-200 bg-blue-50 dark:bg-blue-950/20">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-blue-800 dark:text-blue-200">{geoProcesses.length}</div>
              <p className="text-sm text-blue-600 dark:text-blue-400">Georreferenciamentos ativos</p>
            </CardContent>
          </Card>
          ...
          <Card className="border-green-200 bg-green-50 dark:bg-green-950/20">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-green-800 dark:text-green-200">
                {geoProcesses.filter(p => p.status === "Aprovado").length}
              </div>
              <p className="text-sm text-green-600 dark:text-green-400">Demarcações concluídas</p>
            </CardContent>
          </Card>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Demarcação e coordenadas geográficas - Acompanhe todos os levantamentos
                </CardDescription>
              </div>
              <Badge variant="outline" className="text-blue-700 border-blue-200">
                {geoProcesses.length} processos
              </Badge>
            </div>
          </CardHeader>
          <CardContent>
                                  ...
                                  <TabsContent value="info" className="space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                      <div>
                                        ...
                                        <p className="text-sm">Georreferenciamento</p>
                                      </div>
                                      <div>
        ...
        {/* Informações sobre Georreferenciamento */}
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <h4 className="font-medium text-foreground">O que é Georreferenciamento?</h4>
                <p className="text-sm text-muted-foreground">
                  O georreferenciamento é o processo de determinar as coordenadas geográficas 
                  precisas de uma propriedade rural através de levantamento topográfico.
                </p>
              </div>
              <div className="space-y-2">
                ...
                <ul className="text-sm text-muted-foreground space-y-1">
                  ...
                  <li>• Planta topográfica</li>
                  <li>• Coordenadas geográficas</li>
                  <li>• Levantamento GPS</li>
                </ul>
              </div>
            </div>
          </CardContent>
          ...
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                Upload de Documentos - Georreferenciamento
              </DialogTitle>
            </DialogHeader>
            ...
            {documentsProcess && (
              <div className="space-y-4">
                <div className="p-3 bg-blue-50 rounded-md">
                  <span className="font-medium">Georreferenciamento - {documentsProcess.id}</span>
